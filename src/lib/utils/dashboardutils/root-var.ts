...
// export const getRootVars = (property: any, elem = document.body) => {
//   let _newElem = elem;
...
// };
export const getRootVars = (property: any, elem = document.body) => {
  let _newElem: HTMLElement | null = elem;
  if (_.isString(_newElem)) {
    _newElem = document.querySelector(_newElem);
  }
  if (_newElem) {
    return getComputedStyle(_newElem).getPropertyValue(property).trim() || null;
  }
  return null;
};
...
export const removeRootVariables = (variables: any) => {
  const _root = document.documentElement;
  const _variables = variables;
  _.forEach(_variables, function (_: any, key) {
...
export const setFontFamily = (type: any, fontFamily: any) => {
  // add font into head
  getElems(`[data-font-${type}="google"]`).forEach((elem: any) => {
    elem.remove();
  });
  if (fontFamily === null) {
    fontFamily = "Inter";
  ...
  if (fontFamily !== null && fontFamily !== "Inter") {
    ...
    link.setAttribute(
      "href",
      `https://fonts.googleapis.com/css?family=${fontFamily}:300;400;500;700&display=swap`
    );
